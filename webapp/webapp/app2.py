...
try:
    ...
    topic_name = 'projects/metal-appliance-414300/topics/verify_email'
...
def update_user():
    ...
    if not user.verified:
        app.logger.warning('Username not required')
        return make_response(jsonify({"warning": "Username not required"})), 400  
...
def verify_email():
    ...
    if datetime.utcnow() > expires:
        ...
        return make_response(jsonify({"error": "This verification link has expired."})), 403
